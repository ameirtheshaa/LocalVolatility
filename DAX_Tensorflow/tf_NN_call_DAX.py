    ...
    for date_S0 in dates_S0:
        ...
        class PhysicsModel(tf.keras.Model):
            ...
            def loss_phi_cal(self):
                ...
                phi_tilde_0 = tf.nn.relu(1 - (k_min + (k_max - k_min) * k_tilde_0))
        ...
        class Plotter:
            ...
            def plot_res(self, loss_phi_list, loss_dupire_list, loss_reg_list, error_sigma_list, step):
                ...
                t_tilde_, k_tilde_ = processdata.scale_data(T_nn_, K_nn_)
        ...
        class Reprice:
            ...
            def plot_reprice_paths(self, mc_end, time_taken, t_all, S_matrix, iter_):
                print(f'S_t obtained by solving local volatility SDE M = {M} times from t = [0, {mc_end}], computation time = {time_taken}')
            ...
            def get_S_matrix(self, t_all, S_matrix):
                ...
                T = tf.repeat(tf.reshape(t_all, [-1,1]), m, axis=1)
                K = tf.cast(tf.repeat(tf.reshape(np.linspace(k_min_*S_0, k_max_*S_0, m), [1,-1]), len(T), axis=0), dtype=data_type)
            ...
            def make_plots(self, S_T, T, K, T_nn, K_nn, y_lim, z_lim, iter_):

                def _base_plot(self, val, val_label, savename, z_lim=None):
                    fig = plt.figure(figsize=[8,6], dpi = 450)
                    ax1 = fig.add_subplot(111, projection='3d')
                    ax1.plot_surface(tf.reshape(K, [N,m]), tf.reshape(T, [N,m]), tf.reshape(val, [N,m]), cmap = plt.cm.RdBu_r)
                ...
                def _plots(self):
                    ...
                    self._base_plot(phi_fit_, 'Exact Option Price', 'reprice_phi')
                    print(f'neural sigma')
                    sigma_nn_ = physics.neural_sigma(T_nn, K_nn)
                    self._base_plot(sigma_nn_, 'Volatility', 'reprice_eta', z_lim)
                    print(f'recovered phi from local volatility')
                    phi_rec_ = tf.reshape(self.phi_cal(S_T,T,K), [-1,1])
                    self._base_plot(phi_rec_, 'Repriced Option Price', 'reprice_phi_rec')
                    print(f'rel error')
                    error_ = 100* (phi_fit_ - phi_rec_) / phi_fit_
                    self._base_plot(error_, 'Relative Error (%)', 'reprice_rel_err')

                self.plots_()

            def reprice_RMSE(self, S_matrix):
                ...
                phi_rec = self.phi_cal(S_list_ref, distinct_T, K_list_ref, False)
            ...
            def repricing(self, iter_, y_lim=1, z_lim=0.6, NN_phi_tilde=None, NN_eta_tilde=None, _plots=False):
                ...
                S_T, T, K, T_nn, K_nn = self.get_S_matrix(t_all, S_matrix)
                rmse_rec = self.reprice_RMSE(S_T)
                print ('Reprice RMSE: ', float(np.array(rmse_rec)))
                if _plots:
                    self.plot_reprice_paths(mc_end, time_taken, t_all, S_matrix, iter_)
                    self.make_plots(S_T, T, K, T_nn, K_nn, y_lim, z_lim, iter_)
                ...
                for iter_ in range(num_epochs+1):
                    ...
                    if iter_ % save_epochs == 0 and iter_ != 0:
                        ...
                        if iter_ > int(num_epochs-1):
                            ...
                            reprice_ = reprice.repricing(iter_, physics.NN_phi_tilde, physics.NN_eta_tilde, False)
        ...
        reprice_ = reprice.repricing('final', physics.NN_phi_tilde, physics.NN_eta_tilde, True)
